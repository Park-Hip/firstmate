...
A needs-decision signal row is main-only. In a mixed signal close, routine rows still go to the supervision branch and the excluded decision rows wake this conversation immediately without waiting for branch settlement; stale and heartbeat routing remains unchanged.
