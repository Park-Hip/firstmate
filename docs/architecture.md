...
`bin/fm-watch-arm.sh` remains the verified arm wrapper for protocols that call it; it forks the watcher as a tracked child, verifies it is genuinely alive with a fresh liveness beacon, and reports an honest start, attach, typed live-busy-holder outcome, or failure.
...
A pull-based guard (`bin/fm-guard.sh`) reports checkout tangles and queued main wakes, while [`turnend-guard.md`](turnend-guard.md#guard-predicates) owns its model-aware supervision policy, including repair-before-warning for writable auto-arm sessions and advisory-only behavior for read-only sessions.
