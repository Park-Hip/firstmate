...
A branch offer first reserves its rows through `bin/fm-wake-grant.sh` under that same lock in `state/.branch-reserved-rows`, then publishes the serialized turn's active drain set in `state/.branch-eligible-rows`. Both are bound to the live branch process and extension generation recorded in `state/.branch-eligible-owner`, and reservation is refused if main already claimed any requested row. Main excludes active and queued reservations; the branch drain reads only the active set.
...
A main drain claims every currently unclaimed row and excludes active and queued branch grants from both presentation and acknowledgement.
