...
FM_WATCHER_WEDGE_GRACE=600   # defaults to twice FM_GUARD_GRACE; beacon age past which a LIVE identity-matched holder is a proven wedge the arm reclaims rather than waits for
FM_ARM_BUSY_HOLDER_WAIT=150  # defaults to half FM_GUARD_GRACE; seconds fm-watch-arm waits for a live sub-wedge holder to beat again before reporting the typed busy-holder outcome
FM_CLAUDE_AUTOARM_BUSY_WAIT=150  # defaults to half FM_GUARD_GRACE; seconds one Stop-owned arm attempt waits for a live holder to prove itself before the next attempt
FM_GUARD_AUTOARM_LEDGER_GRACE=7200   # seconds the Claude auto-arm ledger may go quiet, while supervision is needed, before fm-guard attempts its mid-turn self-heal of the Stop-owned owner path
FM_PROCEVENT_RECONCILE_TIMEOUT=30    # seconds allowed for one process-event reconcile pass inside a watcher poll
